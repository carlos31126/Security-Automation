...
public class anotherPoet {

    public static void main(String[] args) throws Exception {
        String filePath = "C:\\Users\\320241893\\OneDrive - Philips\\Desktop\\cci-swcf-business-portal-ms-develop\\cci-swcf-business-portal-ms-develop\\src\\main\\java\\com\\philips\\swcf\\business\\portal\\controller\\BusinessRegistrationController.java";
        List<String> urls = extractUrlsFromController(filePath);
        if (!urls.isEmpty()) {
            for (String url : urls) {
                System.out.println("URL: " + url);
                generateSecurityTestCases(url);
                generateCsrfTest(url);
                generateXssTest(url); // Add this line to generate XSS test case
            }
        } else {
            System.out.println("No URLs found in the controller.");
        }
    }
    ...
    private static void generateCsrfTest(String url) {
        ...
        JavaFile javaFile = JavaFile.builder("com.security.springboot.demosecurity.onemore", testClass)
                ...
                .addStaticImport(MediaType.class,"*")
                .build();
    ...
    private static void generateXssTest(String url) {
        ...
        TypeSpec testClass = TypeSpec.classBuilder("XssTestFor" + capitalize(url.replace("/", "").replace("{", "").replace("}", "")))
                .addModifiers(Modifier.PUBLIC)
                .addAnnotation(AutoConfigureMockMvc.class)
                .addAnnotation(SpringBootTest.class)
                .addMethod(xssProtectionMethod)
                .addField(FieldSpec.builder(MockMvc.class, "mockMvc")
                        .addModifiers(Modifier.PRIVATE)
                        .addAnnotation(Autowired.class)
                        .build())
    ...
    private static String capitalize1(String str){

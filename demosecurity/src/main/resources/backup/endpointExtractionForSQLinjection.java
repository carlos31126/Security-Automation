...
public class endpointExtractionForSQLinjection {
        ...
        if (properties != null && "true".equals(properties.getProperty("checkDosPrevention"))) {
            ...
            if (!urls.isEmpty()) {
                for (String url : urls) {
                    ...
                    generateTestAttemptSQLInjectionShouldReturnForbidden1(url);
                }
            } else {
    ...
    private static void generateTestAttemptSQLInjectionShouldReturnForbidden1(String url) {
        ...
        // Build test class
        TypeSpec testClass = TypeSpec.classBuilder("TestAttemptSQLInjectionShouldReturnForbidden" + capitalize(url.replace("/", "").replace("-", "").replace("{", "").replace("}", "")))
                .addModifiers(Modifier.PUBLIC)

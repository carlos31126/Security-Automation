...
public class endpointExtractionForJWT4{
        ...
        if (properties != null && "true".equals(properties.getProperty("checkDosPrevention"))) {
            ...
            if (!urls.isEmpty()) {
                for (String url : urls) {
                    ...
                    generateTestJwtGenerationLengthandAlgorithm12(url);
                }
            } else {
    ...
    private static void generateTestJwtGenerationLengthandAlgorithm12(String url) {
        String methodName = "testJwtGenerationLengthandAlgorithm12";
        ...
        MethodSpec testMethod = MethodSpec.methodBuilder(methodName)
                ...
                .addStatement("if (tokenParts.length < 2) {\n" +
        ...
        // Build test class
        TypeSpec testClass = TypeSpec.classBuilder("TestJwtGenerationLengthandAlgorithm12"+ capitalize(url.replace("/", "").replace("-", "").replace("{", "").replace("}", "")))
                .addModifiers(Modifier.PUBLIC)
                .addField(mockMvcField)
                .addMethod(testMethod)
                .addMethod(MethodSpec.methodBuilder("isWeakAlgorithm12")
                        .addModifiers(Modifier.PRIVATE)

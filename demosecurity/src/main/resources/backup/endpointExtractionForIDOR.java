...
public class endpointExtractionForIDOR {
        ...
        if (properties != null && "true".equals(properties.getProperty("checkSecurityHeaders"))) {
            ...
            if (!urls.isEmpty()) {
                for (String url : urls) {
                    ...
                    generateTestIdorPrevention(url);
                }
            } else {
    ...
    private static void generateTestIdorPrevention(String url) {
        String methodName = "testIDORPrevention";
        ...
        // Build test class
        TypeSpec testClass = TypeSpec.classBuilder("TestIDORPrevention" + capitalize(url.replace("/", "").replace("-", "").replace("{", "").replace("}", "")))
                .addModifiers(Modifier.PUBLIC)
